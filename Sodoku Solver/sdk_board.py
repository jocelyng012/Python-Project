...
class Board(object):
    ...
    def is_consistent(self) -> bool:
        for group in self.groups:
            used_symbols = set()
            for tile_ in group:
                if tile_.value in CHOICES:
                    if tile_.value in used_symbols:
                        return False
                    else:
                        used_symbols.add(tile_.value)
        return True
    ...
    def naked_single(self) -> bool:
        ...
        for group in self.groups:
            used_symbols = set()
            for tile_ in group:
                if tile_.value not in UNKNOWN:
                    if tile_.value not in used_symbols:
                        used_symbols.add(tile_.value)
            for tile_ in group:
                if tile_.value in UNKNOWN:
                    if tile_.remove_candidates(used_symbols):
                        crossed = True                                
        return crossed
    ...
    def hidden_single(self) -> bool:
        ...
        for group in self.groups:
            ...
            for tile_ in group:
                if tile_.value in CHOICES:
                    leftovers.discard(tile_.value)
            for value in leftovers:
                counter = 0
                for tile_ in group:
                    if tile_.could_be(value):
                        counter += 1
                if counter == 1:
                    for tile_ in group:
                        if tile_.could_be(value):
                            tile_.set_value(value)
                            count = True
        return count
    ...
    def min_choice_tile(self) -> Tile: 
        ...
        for group in self.groups:
            for tile_ in group:
                if tile_.value == UNKNOWN:
                    current = len(tile_.candidates)
                    if current < choice:
                        choice = current
                        pos = tile_
        return pos
